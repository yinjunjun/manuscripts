...
%DIF DEL Paper_R1.tex   Wed Oct 12 13:45:14 2016
%DIF ADD Paper.tex      Fri Oct 14 23:32:21 2016
...
However, it is not clear whether the boundaries truly reflect \DIFdelbegin \DIFdel{people's interactions with the }\DIFdelend \DIFaddbegin \DIFadd{human interactions with }\DIFaddend urban space in \DIFdelbegin \DIFdel{their }\DIFdelend intra- and inter-regional activities.
...
In this study, we presented a method to redraw urban boundaries based upon human interactions with \DIFdelbegin \DIFdel{the }\DIFdelend physical space.
Specifically, we depicted the urban boundaries of Great Britain using a mobility network of Twitter user spatial interactions that was inferred from over 69 million geo-located \DIFdelbegin \DIFdel{Twitter messages}\DIFdelend \DIFaddbegin \DIFadd{Tweets}\DIFaddend .
...
The \DIFdelbegin \DIFdel{technique }\DIFdelend \DIFaddbegin \DIFadd{method }\DIFaddend was applied to both national (Great Britain) and municipal scales (the London metropolis).
...
This well fitted gravity model explains how geographical distances found in the mobility patterns affect the interaction strength among different non-administrative anthropographic urban areas, which provides \DIFdelbegin \DIFdel{a new understanding of }\DIFdelend \DIFaddbegin \DIFadd{new insights into }\DIFaddend the interactions between human activity and urban space.
\newline
...
Different social and \DIFdelbegin \DIFdel{physical }\DIFdelend \DIFaddbegin \DIFadd{spatial }\DIFaddend human interactions were considered to establish the edges of the network connecting the nodes.
...
However, equi-distant tessellation decreases the spatial resolution of aggregated geo-located tweets, because the location information is usually derived from the embedded GPS within mobile devices and tends to provide greater accuracy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{sakaki2010,zandbergen2009}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{zandbergen2009}}%DIFAUXCMD
}\DIFaddend .
...
These patterns include: the number of visited locations per user,  the collective user displacements, and the radius of gyration of individuals in order to identify \DIFdelbegin \DIFdel{natural breaks }\DIFdelend \DIFaddbegin \DIFadd{distinct distance ranges }\DIFaddend in user travel patterns.
We then used these \DIFdelbegin \DIFdel{natural breaks }\DIFdelend \DIFaddbegin \DIFadd{distance ranges }\DIFaddend within the mobility patterns to partition the geographic space of Great Britain into fine-grained cells and established the connectivity among these cells to redraw non-administrative anthropographic urban boundaries. 
...
The probability distribution of the collective displacements can be approximated by \DIFdelbegin \DIFdel{$P(d) \sim \lambda_{1} e^{-\lambda_{1}(d - d_{min})}, d_{min}=10m$ }\DIFdelend \DIFaddbegin \DIFadd{$P(d) \sim \lambda_{1} e^{-\lambda_{1}(d - d_{min})}, d_{min}=10 m$ }\DIFaddend from [\DIFdelbegin \DIFdel{10m, 70m}\DIFdelend \DIFaddbegin \DIFadd{10 m, 70 m}\DIFaddend ] (accounting for 3$\%$ of the population),  $ P(d) \sim \beta\lambda_{1}d^{\beta-1}e^{-\lambda^{1}(d^\beta-d_{min}^\beta)}, d_{min} = 100m$ from [\DIFdelbegin \DIFdel{70m, 70km}\DIFdelend \DIFaddbegin \DIFadd{70 m, 70 km}\DIFaddend ] (93$\%$ of the population), and $P(d) \sim {d}^{-\alpha}$ [$>$ \DIFdelbegin \DIFdel{70km}\DIFdelend \DIFaddbegin \DIFadd{70 km}\DIFaddend ] (4$\%$ of the population). 
The displacement distance between \DIFdelbegin \DIFdel{70m and 100km }\DIFdelend \DIFaddbegin \DIFadd{70 m and 100 km }\DIFaddend can be further approximated by two power law distributions with a cut-off point at \DIFdelbegin \DIFdel{4km }\DIFdelend \DIFaddbegin \DIFadd{4 km }\DIFaddend (55$\%$ distances are less than \DIFdelbegin \DIFdel{4km }\DIFdelend \DIFaddbegin \DIFadd{4 km }\DIFaddend and 40$\%$ distances between \DIFdelbegin \DIFdel{4km and 100km}\DIFdelend \DIFaddbegin \DIFadd{4 km and 100 km}\DIFaddend ), which indicates the urban movement captured by the geo-located Twitter data to reveal two different modes: inter-city and intra-city movement. 
...
The distribution of the radius of gyration of Twitter users in Great Britain can be approximated through a combination of three functions: \DIFdelbegin \DIFdel{$P(r_{g}) \sim \lambda_{2} e^{-\lambda_{2}(r_{g} - {r_{g}}_{min})}, {r_{g}}_{min}=10m$ }\DIFdelend \DIFaddbegin \DIFadd{$P(r_{g}) \sim \lambda_{2} e^{-\lambda_{2}(r_{g} - {r_{g}}_{min})}, {r_{g}}_{min}= 10 m$ }\DIFaddend from [10 m, \DIFdelbegin \DIFdel{30m}\DIFdelend \DIFaddbegin \DIFadd{30 m}\DIFaddend ], $P(r_{g}) \sim \lambda_{2} e^{-\lambda_{2}(r_{g} - {r_{g}}_{min})}$ from [\DIFdelbegin \DIFdel{50m, 10km}\DIFdelend \DIFaddbegin \DIFadd{50 m, 10 km}\DIFaddend ], and $P(r_{g}) \sim {r_{g}}^{-\alpha}$ [\DIFdelbegin \DIFdel{10km, 100km}\DIFdelend \DIFaddbegin \DIFadd{10 km, 100 km}\DIFaddend ], where these three functions account for 92$\%$ of all the users.
...
\DIFaddbegin \DIFadd{To help readers who are not familiar with the geographic context in Great Britain better interpret the derived boundaries, two additional layers (i.e., locations of airport fields and population-wighted-centroids of workplace zones in the Great Britain) are added in the background of the figures.
}\DIFaddend Fig.~\ref{S5_Fig} presents the delineated urban boundaries based on Twitter user displacement distance less than 4 km, greater than \DIFdelbegin \DIFdel{4km}\DIFdelend \DIFaddbegin \DIFadd{4 km}\DIFaddend , greater than 10 km, and using all available displacements together compared to the administrative boundaries of Great Britain.
...
\begin{figure}[ht]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\linewidth]{./figure/S5_community}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.85\linewidth]{./figure/S5_community}
\DIFaddendFL \caption{{\bf The community structure from collective Twitter user displacements reveals non-administrative anthropographic urban boundaries} (a) all displacements with $L(M)= 7.8$. (b) displacements longer than 10 km ($L(M)=8.5$)\DIFaddbeginFL \DIFaddFL{, red symbols are the locations of airport fields in Great Britain }\DIFaddendFL (c) displacements shorter than 4 km ($L(M)=4.5$) (d) and displacements longer than 4 km ($L(M)=8.1$).The partition of space was done using a 10 km fishnet for the directed displacements from and to each cell. Each color represents a unique community with more Twitter users displacements among the cells compared to others. Major cities (urban audit functional areas) and NUTS are displayed as labels.}
\label{S5_Fig}
\end{center}
\end{figure}
...
\begin{figure}[ht]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\linewidth]{./figure/S6_community_london}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\linewidth]{./figure/S6_community_london}
\DIFaddendFL \caption{{\bf The non-administrative anthropographic regions inferred from Twitter user displacements greater than 4 km (left) and 10 km (right) in comparison with major cities in England (upper figures) and Scotland (bottom figures).}  Each color represents a unique community. Including short distance movements has increased the power to differentiate the influence of nearby cities such as Glasgow and Edinburgh (lower left), while restricting the analysis to longer distance movements grouped travelers from the two previous cities into the same community (lower right). \DIFaddbeginFL \DIFaddFL{Red symbols are the locations of airport fields and gray points are the population-wighted-centroids of workplace zones in Great Britain.}\DIFaddendFL }
\label{S6_Fig}
\end{center}
\end{figure}
...
The spatial partitions derived from a fine grid of \DIFdelbegin \DIFdel{1km }\DIFdelend \DIFaddbegin \DIFadd{1 km }\DIFaddend used all available Twitter user trips without any restriction on trip distances yields geographic boundaries comparable to some of London's boroughs  (Fig.~\ref{S7_Fig}).
...
\begin{figure}[ht]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\linewidth]{./figure/S7_london}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\linewidth]{./figure/S7_london}
\DIFaddendFL \caption{{\bf Non-administrative anthropographic boundaries inferred from collective Twitter user displacements in the city of London compared to the boundaries of London boroughs ($L(M)=8.1$)}  A fine fishnet of 1 km cells were used to identify the detailed connectivity patterns based on all the Twitter user displacements in the area.  Each unique color represents a different non-administrative anthropographic region. Notice that some remote regions like the airport (light purple region in south of Hillingdon) share the same class with downtown because it is well connected despite the geographic separation. \DIFaddbeginFL \DIFaddFL{Red symbols are the locations of airport fields and gray points are the population-wighted-centroids of workplace zones in London.}\DIFaddendFL }
\label{S7_Fig}
\end{center}
\end{figure}
...
The probability distributions of the radius of gyration for Twitter users in \DIFdelbegin \DIFdel{the }\DIFdelend Great Britain are verified to be consistent across different monthly time span (see Supplement Materials section 2), which indicates the stability of using such measurements in this study.  
...
The probability of distance that deviates from a user's center location decays with a stretched-exponential function from [\DIFdelbegin \DIFdel{50m, 10km}\DIFdelend \DIFaddbegin \DIFadd{50 m, 10 km}\DIFaddend ], which means the movements from Twitter users with smaller spatial coverage dominate the delineation of the connected urban areas.
...
\section{Conclusion}
In this study, we \DIFdelbegin \DIFdel{presented }\DIFdelend \DIFaddbegin \DIFadd{described }\DIFaddend a method that connected human mobility research with the delineation of the non-administrative anthropographic urban boundaries in Great Britain using a mobility network of Twitter user spatial interactions.
In contrast to administrative urban boundaries, our ``bottom-up" approach \DIFdelbegin \DIFdel{imposed }\DIFdelend \DIFaddbegin \DIFadd{constructed }\DIFaddend a virtual fishnet over the islands of Great Britain to partition the space.
...
\DIFdelbegin \DIFdel{Then we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend applied the map equation algorithm to partition the network and associated geographic regions.
The strongly connected communities within the network space \DIFdelbegin \DIFdel{yields }\DIFdelend \DIFaddbegin \DIFadd{yielded }\DIFaddend geographically cohesive, non-overlapping urban areas that \DIFdelbegin \DIFdel{provides }\DIFdelend \DIFaddbegin \DIFadd{provided }\DIFaddend a clear delineation of the urban boundaries in Great Britain.
...
Identifying the connected regions at each of these distance ranges \DIFdelbegin \DIFdel{yielded }\DIFdelend \DIFaddbegin \DIFadd{revealed }\DIFaddend hierarchical boundaries of  the urban space in Great Britain.

The power of using Twitter user mobility to delineate non-administrative anthropographic boundaries \DIFdelbegin \DIFdel{is the ability to redraw the city }\DIFdelend \DIFaddbegin \DIFadd{enables urban representation }\DIFaddend at different mobility ranges inferred objectively from the collective mobility patterns. 
Urban boundaries redrawn based on Twitter user movement represent physical commutes rather than social ties or phone call initiation to reflect \DIFdelbegin \DIFdel{the }\DIFdelend human interactions with the geographical space.
More importantly, as the depicted urban boundaries exhibited a strong instance of spatial proximity, we further employed a gravity model to connect human mobility research to understand and justify the distance decay effects in shaping the delineated urban boundaries.
This well fitted gravity model explains how geographical distances found in the mobility patterns affect the interaction strength among different non-administrative anthropographic urban areas, \DIFdelbegin \DIFdel{which provides a new understanding of }\DIFdelend \DIFaddbegin \DIFadd{gaining new insights into }\DIFaddend the interactions between human activities and urban space.
A final remark from this study for future \DIFdelbegin \DIFdel{studies }\DIFdelend \DIFaddbegin \DIFadd{work }\DIFaddend is that geographical distance plays an important role in affecting human mobility patterns and the strength of human spatial interactions across space.
Our approach can be considered to assist in understanding human spatial interactions from the mobility perspective, which is applicable \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{based on }\DIFaddend detailed geo-located Twitter data in \DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{many }\DIFaddend countries, as well as future mobility datasets with detailed location information of individuals and large spatial coverage.  
...
%DIFDELCMD < \bibitem[\protect\citeauthoryear{Sakaki {\itshape{et~al.}}}{2010}]{sakaki2010}
%DIFDELCMD < %%%
\DIFdel{Sakaki, T., Okazaki, M., Matsuo, Y., 2010. Earthquake Shakes Twitter Users: Real-time Event Detection by Social Sensors, in: Proceedings of the 19th International Conference on World Wide Web, WWW ’10. ACM, NY, USA, pp. 851-860
}\DIFdelend 
