...
%	\author[1, 2, 3, 4]{Shaowen Wang}	
%%	\author[1, 2, 3, 4, \footnote{Corresponding author: shaowen@illinois.edu}]{Shaowen Wang}
%	
%	\affil[1]{
...
In particular, the Twitter user mobility patterns measured by the displacements and radius of gyrations of individuals have revealed different groups of Twitter users with multi-scale or multi-modal mobility patterns.
...
Notes: What about randomly choose several states and perform the analysis
\cleardoublepage
...
%This section is divided into two parts. The first part presents a set of statistical analysis to explore Twitter user mobility patterns (e.g., movement flows, user visiting behavior, displacements and radius of gyrations) across multi-level spatial scales and temporal granularity in the United States during the year 2014, where all the measurements are enabled by the framework.  
%The second part presents the details of the interactive 3D virtual globe web mapping interface for exploratory geo-visual analytics of the multi-level spatiotemporal Twitter user movements. 
%In particular, the interface provides capability to dynamically visualize the movements and summarize movement flows based on a given spatial scale and time range, which are specified by the users when they are interacting with the web interface.
...
%In addition to providing supports for understanding Twitter user mobility patterns with statistical analysis, our framework integrates a 3D virtual globe web mapping interface to enable users to perform exploratory geo-visual analytics of the multi-level spatiotemporal Twitter user movements. 
...
%%\begin{figure}[h]
%%\centering
%%\includegraphics[width=0.7\linewidth]{./figures/vaca_globe}
%%\caption{3D extruded polygons with heights proportional to the volume of incoming Twitter user movements in each state }
%%\label{fig:vaca_movement}
%%\end{figure}
%%\FloatBarrier
