...
A common outcome observed from the mentioned studies is that the strongly connected urban regions in the form of communities in the network space yield geographically cohesive urban areas, in spite of different community detection methods and various forms of social and physical human interactions were used.
The general consensus is that those geographically cohesive urban areas are instances of spatial proximity effects, where the interaction strength between two urban regions decreases as the geographical distance between them increases~\citep{fotheringham1981}.
...
While it is intuitively logical, few research efforts were further carried out to quantitatively understand how the spatial interactions shape the forms of connected urban areas (i.e., urban boundaries). One of the reason may due to the      
it is not an explicit constraints in these 
