...
%DIF DEL giscience2018.tex       Wed Feb 21 07:38:20 2018
%DIF ADD giscience2018_new.tex   Thu Feb 22 01:57:31 2018
...
%DIF 30c30
%DIF < \affil[1]{Department of Civil Engineering,  Gwangju, Republic of Korea\\
%DIF -------
\affil[1]{Department of Civil Engineering, Chosun University, Gwangju, Republic of Korea\\ %DIF > 
%DIF -------
  \texttt{mhjeong@chosun.ac.kr}}
%DIF 32c32
%DIF < \affil[2]{Social Science Research Institute, Penn State University, PA, USA\\
%DIF -------
\affil[2]{Social Science Research Institute; Institute for CyberScience, Penn State University, PA, USA\\ %DIF > 
%DIF -------
  \texttt{jyin@psu.edu}}
%DIF 34c34
%DIF < \affil[3]{Departmet of Geography and Geographic Information Science, the University of Illinois at Urbana-Champaign, IL, USA\\
%DIF -------
\affil[3]{Departmet of Geography and Geographic Information Science, University of Illinois at Urbana-Champaign, IL, USA\\ %DIF > 
%DIF -------
...
%DIFDELCMD < %%%
\DIFdelend With the rapid rise in ubiquity of \DIFdelbegin \DIFdel{geo-tagged }\DIFdelend \DIFaddbegin \DIFadd{geolocation-aware }\DIFaddend sensors, knowledge discovery \DIFdelbegin \DIFdel{power is }\DIFdelend \DIFaddbegin \DIFadd{is greatly }\DIFaddend enhanced by extracting \DIFaddbegin \DIFadd{and mining }\DIFaddend interesting patterns from spatiotemporal \DIFdelbegin \DIFdel{big data }\DIFdelend \DIFaddbegin \DIFadd{Big Data }\DIFaddend in various domains.
In particular, the location-acquisition technologies generate large volumes of movement data\DIFdelbegin \DIFdel{such as }\DIFdelend \DIFaddbegin \DIFadd{, which were used to track }\DIFaddend people, animals, vehicles\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and even }\DIFaddend natural phenomena.
\DIFdelbegin \DIFdel{These big movement }\DIFdelend \DIFaddbegin \DIFadd{Such }\DIFaddend data help us \DIFdelbegin \DIFdel{understand moving objects , and discover the hidden patterns . Thus}\DIFdelend \DIFaddbegin \DIFadd{better model moving objects and reveal hidden patterns that are important to urban planning and its applications in characterizing urban human mobility \mbox{%DIFAUXCMD
\cite{jiang2012discovering}}%DIFAUXCMD
, accessibility of the activity space \mbox{%DIFAUXCMD
\cite{kwan1998space}}%DIFAUXCMD
, and sustainability of the urban systems \mbox{%DIFAUXCMD
\cite{alberti2003integrating}}%DIFAUXCMD
. 
Importantly}\DIFaddend , trajectory mining leads to solutions for \DIFaddbegin \DIFadd{addressing }\DIFaddend important research problems in different fields\DIFaddbegin \DIFadd{, }\DIFaddend such as urban planning \cite{mazimpaka15AGILE}, transportation \cite{chen13Percom}, environment \cite{devarakonda13SIGKDD}, and public security and safety \cite{buchin14JOSIS}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend This paper presents a new algorithm which not only estimates origination-destination (OD)'s flows anomaly but also conducts hypothesis testing between two \DIFaddbegin \DIFadd{sets of }\DIFaddend different OD flows. \DIFdelbegin \DIFdel{This paper utilizes a }\DIFdelend \DIFaddbegin \DIFadd{The algorithm is applied on the }\DIFaddend New York City \DIFdelbegin \DIFdel{Taxi data setthat has }\DIFdelend \DIFaddbegin \DIFadd{taxi data set, where each record contains }\DIFaddend the origin and destination of each \DIFdelbegin \DIFdel{movement without the whole actual trajectory routes. The analysis of digital footprints obtained from taxi }\DIFdelend \DIFaddbegin \DIFadd{trip (without intermediate locations of the actual routes).
We believe the our methods for analyzing taxi trip }\DIFaddend data has a \DIFdelbegin \DIFdel{crucial role }\DIFdelend \DIFaddbegin \DIFadd{promising application }\DIFaddend in understanding crowd patterns and planning urban and transportation investments \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by the }\DIFaddend administrative authorities.

In recent years, researchers have investigated a variety of approaches to trajectory data mining.
\DIFdelbegin \DIFdel{There are generally four categories that classify }\DIFdelend \DIFaddbegin \DIFadd{In general, the contemporary }\DIFaddend trajectory mining methods \DIFaddbegin \DIFadd{can be classified into four categories}\DIFaddend : clustering, classification, frequent/group pattern mining, and outlier detection \cite{mazimpaka16JOSIS,zheng15ACMTIST}. These techniques can be used independently or combinatorially for trajectory mining \DIFdelbegin \DIFdel{application problems. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In particular, this studyfocuses }\DIFdelend \DIFaddbegin \DIFadd{applications. For this study, we focus }\DIFaddend on outlier detection of OD flows. Outlier detection aims to find trajectories that do not follow the typical flow of trajectory data sets for characterizing the connectivity between regions \cite{mazimpaka16JOSIS}. \cite{fontes13GeoInfo,lee08ICDE} use Euclidean distance to find outlier patterns from trajectories. \cite{pan13ACMGIS,liu12IJGIS} raise the questions of Euclidean distance approach due to the loss of local features and unavailability when external factors affect the trajectories (e.g., topography, land cover or weather condition).  \cite{pan13ACMGIS,liu12IJGIS} addresses this by using robust distance measurements (i.e., Mahalanobis distance \cite{pan13ACMGIS} and relative distance \cite{liu12IJGIS}). Further, structural features \cite{yuan11JCIS} and a data-induced random tree  \cite{zhang11UC} are exploited to detect anomalous trajectories instead of using distance or density. In a similar fashion, this study considers the center regions of OD flows with data depth in order to robustly detect the trajectory outliers. 

In addition, \DIFdelbegin \DIFdel{visual analytics are }\DIFdelend \DIFaddbegin \DIFadd{utilizing visual analytics is }\DIFaddend a common approach to analyze OD flow data.
...
While visual analytics can help to extract inherent patterns from massive data, it is difficult to \DIFdelbegin \DIFdel{compare two }\DIFdelend \DIFaddbegin \DIFadd{quantatively compare two sets of }\DIFaddend different OD flows based on a hypothesis testing.
In other words, it is complicated to comprehend how two OD flows differ and\DIFaddbegin \DIFadd{, more importantly, }\DIFaddend by how much.
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{In this light, this }\DIFaddend paper uses bivariate hypothesis testing methods based on data depth to understand how different two OD flow data sets are in terms of the amount of spatial extent and a shift in location. 

\DIFdelbegin \DIFdel{Further, }\DIFdelend \DIFaddbegin \DIFadd{It is worthy noting that }\DIFaddend flow mapping approaches frequently suffer from the modifiable \DIFdelbegin \DIFdel{area }\DIFdelend \DIFaddbegin \DIFadd{areal }\DIFaddend unit problem (MAUP). For instance, it is not guaranteed that different aggregations via location can present coherent patterns.
Kernel-based flow estimation and smoothing are used to overcome different spatial resolution \cite{guo14IEEETVCG}. 
\DIFdelbegin \DIFdel{This study uses }\DIFdelend \DIFaddbegin \DIFadd{Instead of focusing on finding the best areal unit to partioning the urban space and aggregating the OD flows, this study chooses }\DIFaddend traffic analysis zones in New York City as a base unit. \DIFdelbegin \DIFdel{Thus, it does not consider MAUP problem}\DIFdelend \DIFaddbegin \DIFadd{However, the mothods can be adapted to other choices of areal units}\DIFaddend .
In addition, New York City \DIFdelbegin \DIFdel{Taxi data includes }\DIFdelend \DIFaddbegin \DIFadd{taxi trip data include }\DIFaddend the origin and destination within traffic analysis zones, which ignores the \DIFdelbegin \DIFdel{actual trajectory route}\DIFdelend \DIFaddbegin \DIFadd{intermediate locations of the actual routes}\DIFaddend .
It is not necessary to reconstruct individual movements for flow estimation (see \cite{duckham16ICGIS}).

In summary, this paper presents a new algorithm which conducts outlier detection as well as a hypothesis testing from OD flows data. Our approach investigates the central regions of OD flows based on data depth to detect OD flows anomaly and conduct a hypothesis testing between two different OD flow data sets. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend remainder of this paper \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{is organized as follows: }\DIFaddend Section \ref{sec:methods} overviews how to detect OD flows outliers and conduct  a hypothesis testing between two different OD flows with the concept of data depth. 
